...
from datasets import ImageDataset,ImageDataset_eval
import os
import skimage
import pytorch_ssim
...
parser.add_argument('--dataroot', type=list, default=['../low_high_datasets/PNLI/train/'], help='root directory of the dataset')
parser.add_argument('--dataeval', type=list, default=['../low_high_datasets/PNLI/eval/'], help='root directory of the dataset')
...
result_dir = "./result_PNLI/"
checkpoint_dir = "./output_PNLI/"
result_test_dir = "./result_PNLI/test"
...
for epoch in range(opt.epoch, opt.n_epochs):
    ...
    for i, batch in enumerate(dataloader):
        ...
        loss_identity_B = criterion_identity(fake_B, real_B)*5.0

        pred_fake = netD_B(fake_B)

        loss_GAN_A2B = criterion_GAN(pred_fake.squeeze(), target_real)

        loss_ssim_AB = ssim_loss(fake_B, real_B)

        loss_vgg_AB = vgg_loss(fake_B, real_B) * 0.1
        # Total loss
        ...
        loss_G = loss_GAN_A2B + loss_identity_B #+ loss_ssim_AB+loss_vgg_AB
        loss_G.backward()
        ...
        if epoch % 50 == 0:
            ...
            rcA = real_B.permute(0, 2, 3, 1).data.cpu().numpy()
            saveA = np.concatenate((rA[0,:,:,:], fB[0,:,:,:], rcA[0,:,:,:]), axis=1)
            saveimage(saveA, result_dir, epoch, i)
        ###################################
        ###### Discriminator B ######
        ...
        optimizer_D_B.step()
        print("epoch: %d, batch: %d, loss_G: %.3f, loss_D: %.3f" % (epoch, i, loss_G,  loss_D_B))
        ###################################
    ...
    if epoch%20==0:
        ...
        s_psnr = 0
        s_ssim = 0
        low = Tensor(opt.batchSize, opt.input_nc, opt.size, opt.size)
        gt = Tensor(opt.batchSize, opt.input_nc, opt.size,opt.size)
        with torch.no_grad():
